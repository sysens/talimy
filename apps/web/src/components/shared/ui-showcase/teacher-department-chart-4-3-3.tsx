    ...
    <div className="space-y-3">
      ...
      <div className="max-w-sm rounded-[28px] bg-card p-5">
        <div className="flex items-start justify-between gap-3">
          <h4 className="text-base font-semibold text-talimy-navy">Department</h4>
          <button
        ...
        <div className="mt-4 border-t border-[color:color-mix(in_srgb,var(--talimy-color-gray)_25%,white_75%)] pt-4">
          <div className="space-y-3">
            {DEPARTMENT_STATS.map((item) => (
              <div
                key={item.label}
                className="grid grid-cols-[minmax(0,1fr)_32px_40px] items-center gap-3"
              >
                ...
                <span className="text-right text-sm text-[var(--talimy-color-gray)]">
                  {item.count}
                </span>
                <span className="text-right text-sm font-semibold text-talimy-navy">
                  {item.percent}
                </span>
              </div>
            ))}
